...
export default function UserOrder() {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 max-w-xl mx-auto">
      <h3 className="font-bold text-base mb-2">สรุปรายการ</h3>
      {/* ลิสต์สินค้า */}
        ...
        {cartsForPayment.map((item) => (
          <div key={item.id} className="flex items-center gap-3 border rounded p-2">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-gray-500 mt-1">ราคา: <span className="text-green-600 font-semibold">{item.final_price_product?.toLocaleString()} ฿</span></div>
            </div>
          </div>
        ))}
      </div>
