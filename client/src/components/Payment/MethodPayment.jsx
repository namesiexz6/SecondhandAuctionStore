...
export default function MethodPayment({ value, onChange }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 max-w-xl mx-auto">
      <h3 className="font-bold text-base md:text-lg mb-3">Select Payment Method</h3>
      {/* Show only selected method */}
      <div className="flex items-center gap-3 p-3 border rounded mb-3">
        <span className="text-xl md:text-2xl">{selectedMethod.icon}</span>
        <span className="font-medium text-sm md:text-base">{selectedMethod.label}</span>
        <button
          className="ml-auto px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded font-medium text-xs md:text-sm"
          type="button"
      ...
      {showPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg p-6 w-full max-w-md z-10">
            <h4 className="font-bold text-base md:text-lg mb-4">Select Payment Method</h4>
            <div className="flex flex-col gap-3">
              {paymentMethods.map((method) => (
                ...
                >
                  ...
                  {!method.enabled && (
                    <span className="ml-2 text-xs md:text-sm text-gray-400">(Coming soon)</span>
                  )}
                </label>
              ))}
            </div>
            <button
              className="mt-5 w-full bg-gray-300 hover:bg-gray-400 text-gray-700 py-2 rounded font-medium text-xs md:text-sm"
              type="button"
